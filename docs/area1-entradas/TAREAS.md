...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | Integración con el área 3: acordar D-03 y abrir I-01 (fórmula de capital en `fn_entrada_producto` y `v_entradas_area1`); verificarla una vez aplicada | tipo:integracion | nivel:medio | S | — | Líder | RN-A1-02, RN-A1-08 | #20 |
| 2 | Integración con el área 5: abrir I-04 (`v_entradas_importacion` a cambio de impuestos aduanales por entrada) y confirmar que la vista le sirve | tipo:integracion | nivel:inicial | S | — | Líder | RN-A1-08 | #21 |
| 3 | Validación con Almacén y Operaciones: checklist de la sección 11.5 y cierre de las preguntas abiertas 1, 6 y 8 | tipo:docs | nivel:inicial | S | — | Docs | — | #22 |
| 4 | Migración de tipos y catálogos: enums `estado_mercancia`, `tipo_almacen`, `estado_orden`, `resultado_calidad`; tablas `proveedores` y `almacenes` con RLS ★ | tipo:bd | nivel:inicial | S | 1 | Datos | RN-A1-06 | #23 |
| 5 | Migración aditiva de `productos` y `entradas_producto`: columnas de SKU y catálogo, columnas de origen, recepción y valor; `CHECK` de moneda y tipo de cambio; `CHECK` de documento de respaldo; índice único parcial de SKU | tipo:bd | nivel:medio | M | 4 | Datos | RN-A1-01, RN-A1-02, RN-A1-07, RN-A1-18 | #24 |
| 6 | Migración de manufactura: `lotes`, `materias_primas`, `entradas_materia_prima`, `ordenes_produccion`, `bom`, `consumo_produccion`, `control_calidad`, con sus `CHECK`, `UNIQUE (id_orden_produccion)` en entradas y RLS en todas las tablas nuevas | tipo:bd | nivel:medio | L | 4 | Datos | RN-A1-05, RN-A1-09, RN-A1-12, RN-A1-15 | #25 |
| 7 | Seed `seed_area1.sql`: 2 almacenes, 4 proveedores, 6 productos con SKU, 3 materias primas, 1 BOM, entradas del ejemplo 10.1, una importación en USD y una orden que llega a `TERMINADA` (ejemplo 10.2) | tipo:pruebas | nivel:medio | M | 5, 6 | Docs | RN-A1-08 | #26 |
| 8 | Trigger `fn_valida_entrada`: almacén de producto terminado, lote del mismo producto, copia del nombre del proveedor, documento de respaldo | tipo:bd | nivel:avanzado | M | 5, 6 | Datos | RN-A1-04, RN-A1-05, RN-A1-06, RN-A1-18 | #27 |
| 9 | Triggers de materias primas y de orden: `fn_entrada_materia`, `fn_valida_almacen_materia`, `fn_valida_orden` (producto con BOM), `fn_transicion_orden` (máquina de estados sin retroceso) | tipo:bd | nivel:avanzado | M | 6 | Datos | RN-A1-06, RN-A1-09, RN-A1-10, RN-A1-11 | #28 |
| 10 | Triggers de consumo y calidad: `fn_consumo_materia` (solo `EN_PROCESO`, materia en el BOM, sin stock negativo, marca de desviación mayor a 20 %) y `fn_valida_calidad` (totales, motivo, responsable distinto) | tipo:bd | nivel:avanzado | M | 9 | Datos | RN-A1-09, RN-A1-13, RN-A1-14, RN-A1-15, RN-A1-16 | #29 |
| 11 | Trigger `fn_cerrar_orden`: al pasar a `TERMINADA` calcula costo unitario de manufactura e inserta la entrada de producto terminado con proveedor "PRODUCCIÓN INTERNA" | tipo:bd | nivel:avanzado | M | 8, 10 | Datos | RN-A1-12, RN-A1-17 | #30 |
| 12 | Vistas de reporte: `v_entradas_detalle`, `v_stock_por_almacen`, `v_discrepancias_recepcion`, `v_reabastecimiento`, `v_capital_en_proceso` | tipo:bd | nivel:medio | M | 8, 11 | Datos | RN-A1-08 | #31 |
| 13 | Vistas de contrato: `v_entradas_importacion` (área 5), `v_ordenes_produccion` (área 6), `v_mano_obra_produccion` (área 4) | tipo:bd | nivel:medio | M | 11 | Datos | RN-A1-14, RN-A1-17 | #32 |
| 14 | Pruebas SQL del área en `supabase/tests/area1/`: entradas (almacén, lote, moneda, documento, proveedor) y manufactura (orden sin BOM, retroceso, consumo, stock negativo, calidad, doble cierre, costo del ejemplo 10.2) | tipo:pruebas | nivel:avanzado | L | 7, 11 | Docs | RN-A1-04 a RN-A1-18 | #33 |
| 15 | Servicios `src/services/area1/`: `proveedores.js`, `almacenes.js`, `entradas.js`, `produccion.js`, con manejo de errores de Postgres a mensajes legibles | tipo:backend | nivel:avanzado | L | app base de coordinación, 8, 11 | Lógica | RN-A1-03, RN-A1-08, RN-A1-09 a RN-A1-17 | #34 |
| 16 | Módulo del área en la app compartida (`src/modules/area1-entradas/`, ruta "Entradas" del menú) y pantallas de catálogos de proveedores y almacenes: listado, alta, edición y baja lógica ★ | tipo:frontend | nivel:inicial | M | 15 | Frontend | RN-A1-06 | #35 |
| 17 | Pantalla de entradas (vía A): formulario de registro con capital estimado en vivo, y listado con 3 `KpiCard` y `Table` desde `v_entradas_detalle`; manejo del rechazo del trigger sin perder lo capturado | tipo:frontend | nivel:avanzado | L | 15, 16 | Frontend | RN-A1-01, RN-A1-07, RN-A1-08, RN-A1-18 | #36 |
| 18 | Pantalla de orden de producción: alta con BOM y consumo teórico, captura de consumo y control de calidad con botones habilitados según estado, y cierre que muestra el costo unitario y la entrada generada | tipo:frontend | nivel:avanzado | L | 15, 16 | Frontend | RN-A1-10 a RN-A1-17 | #37 |
| 19 | Pruebas de punta a punta desde la app: entrada del ejemplo 10.1 hasta `v_inventario_actual` y orden del ejemplo 10.2 hasta el inventario; capturas en `evidencias/` | tipo:pruebas | nivel:medio | M | 17, 18 | Docs | RN-A1-08, RN-A1-17 | #38 |
| 20 | Integración con las áreas 4 y 6: confirmar que consumen `v_mano_obra_produccion` y `v_ordenes_produccion`; verificar que `v_entradas_area1` cuadra con `v_entradas_detalle` | tipo:integracion | nivel:inicial | S | 13, 1 | Líder | — | #39 |
| 21 | Cierre del área: evidencias por issue, `ESTADO.md` diario, `CONTEXTO.md` actualizado con los acuerdos reales y preguntas abiertas cerradas, reporte final y evaluación final del equipo | tipo:docs | nivel:inicial | M | todo | Líder, Docs | — | #40 |
