...
> Unifica `proyecto_comercializadora.md` (contexto y flujos) con `modelo_er_comercializadora.md` (esquema final).
> Ante cualquier contradicción entre ambos, **gana el esquema de `modelo_er_comercializadora.md`**, que es el modelo aprobado.
...
5. Las reglas de negocio están numeradas (`RN-01` … `RN-09`). Cítalas por su identificador en comentarios de código, mensajes de commit y descripciones de PR.
...
| `MARKETING`, `CLIENTES_MARKETING` | Solo lectura | Propiedad del área 6 |
...
| `stock` | INT | No | Existencia disponible ahora. Default 0. Nunca negativo (RN-01). |
...
| `cantidad` | INT | No | Unidades recibidas. Siempre > 0 (RN-04). |
...
Se conserva `PRODUCTOS.stock` como columna (no se calcula al vuelo) porque así lo define el modelo aprobado. La consistencia se garantiza con triggers (RN-06): ninguna aplicación escribe `stock` directamente.
...
La captura es de una sola fila en `ENTRADAS_PRODUCTO`. Los cuatro efectos sobre `PRODUCTOS` los ejecuta el trigger. **Una aplicación que actualice `stock` por su cuenta duplica el incremento** — ese es el bug clásico de este diseño, y el motivo de RN-06.
...
|---|---|---|
| **RN-01** | `PRODUCTOS.stock` nunca puede quedar negativo. Ninguna venta se completa si dejaría el stock bajo cero. | `CHECK (stock >= 0)` + trigger `fn_salida_producto` |
| **RN-02** | Toda entrada y toda salida debe referirse a un `id_producto` existente. | Llaves foráneas |
| **RN-03** | Una factura debe contener al menos un renglón de detalle (cardinalidad 1..N). | Validación de aplicación |
| **RN-04** | `cantidad` siempre mayor que cero, tanto en entradas como en salidas. | `CHECK (cantidad > 0)` |
| **RN-05** | `volumen` es acumulado histórico y nunca decrece. Solo lo incrementan las entradas. | Trigger de entrada |
| **RN-06** | Ninguna aplicación escribe `PRODUCTOS.stock` directamente. Solo lo modifican los triggers de entrada, salida y ajuste. | Convención + revisión de código |
| **RN-07** | Un ajuste de inventario conserva `stock_sistema` y `conteo_fisico` como evidencia. Nunca se corrige el stock sin dejar el registro del ajuste. | Trigger `fn_ajuste_inventario` |
| **RN-08** | `valor_entrada` refleja el costo de la **última** entrada; el histórico de costos vive en `ENTRADAS_PRODUCTO`. | Trigger de entrada |
| **RN-09** | El área 3 no modifica `FACTURAS`, `CLIENTES`, `AGENTES_VENTAS`, `MARKETING` ni `IMPUESTOS_LICENCIAS`. | Políticas RLS + revisión de código |
...
CREATE TABLE productos (
    ...
    CONSTRAINT ck_stock_no_negativo CHECK (stock >= 0),          -- RN-01
    CONSTRAINT ck_volumen_no_negativo CHECK (volumen >= 0),      -- RN-05
...
CREATE TABLE entradas_producto (
    ...
    CONSTRAINT ck_entrada_cantidad CHECK (cantidad > 0),         -- RN-04
...
CREATE TABLE detalle_factura (
    ...
    CONSTRAINT ck_detalle_cantidad CHECK (cantidad > 0)          -- RN-04
);
...
```sql
-- ENTRADA: actualiza stock, volumen, capital invertido y último costo (RN-05, RN-08)
...
-- SALIDA: valida existencia y descuenta (RN-01)
...
BEGIN
    ...
    IF v_stock < NEW.cantidad THEN
        RAISE EXCEPTION 'RN-01: stock insuficiente para "%" (disponible: %, solicitado: %)',
...
-- AJUSTE: alinea el stock al conteo físico dejando evidencia (RN-07)
...
/docs
  AREA3_inventario_contexto.md   ← este archivo
/src
...
- Commits: `area3: <verbo> <objeto>` — por ejemplo `area3: agrega trigger de conciliación (RN-07)`.
...
// Registrar una entrada — el trigger hace el resto, NO actualices stock (RN-06)
...
2. ¿El costo de inventario es último costo o promedio ponderado? Actualmente `valor_entrada` guarda el último (RN-08); contabilidad podría requerir promedio.
...
| 2 | Modelo ER validado y contratos acordados con áreas 1 y 2 |
| 3 | Migraciones 0001–0003 aplicadas en Supabase |
| 4 | Triggers y RLS funcionando; pruebas de RN-01 y RN-04 |
| 5 | Vistas listas y carga de datos reales |
